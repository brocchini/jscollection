				...
				function() {
					...
					test("Empty collection test",
							function() {
								...
								ok(col.isEmpty(),
										'isEmpty() Expected [true], got: '
												+ col.isEmpty());

								equals(col.size(), 0, 'size() got: '
										+ col.size());

								ok(col.hasNext() === false,
										'hasNext() Expected [false], got: '
												+ col.hasNext());

								equals(col.get(d1.obj1), undefined,
										'get(obj1) got: ' + col.get(d1.obj1));

								equals(col.get(1), undefined, 'get(1) got: '
										+ col.get(1));
					...
					test("Enter data test",
							function() {
								var d2 = new makeData();
								var col = d2.col;

								equals(col.get(0), d2.obj1, 'get(0) got: '
										+ col.get(0));

								ok(col.isEmpty() === false,
										'isEmpty() Expected[false], got: '
												+ col.isEmpty());

								equals(col.size(), 3, 'size() got: '
										+ col.size());

								ok(col.hasNext() === true,
										'hasNext() Expected [true], got: '
												+ col.hasNext());

								var nextItem = col.next();
								equals(nextItem, d2.obj1, 'next() got: '
										+ nextItem);

								equals(col.contains(d2.obj1), true,
										'contains(d2.obj1) got: '
					...
					test(
							...
							function() {
								...
								equals(col.get(0), d3.obj3, 'col.get(0) got: '
										+ col.get(0));
					...
					test("each sum all test", function() {
						...
						col.each(sum);
						equals(total, 8 + 2 + 5, '"[8,2,5]" each(sum) got: '
								+ total);
					});

					test("flatten test", function() {
						var col = new Collection([ 'January', 'February',
								'March' ]);
